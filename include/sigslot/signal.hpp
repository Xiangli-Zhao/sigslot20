...
template <GroupId, typename, typename...> class signal_base;
...
template <trait::Function T> struct function_traits<T> {
...
template <trait::MemFnPointer T> struct function_traits<T> {
...
// operator
template <trait::Functor T> struct function_traits<T> {
...
template <typename T> struct object_pointer {
...
template <trait::WeakPtr T> struct object_pointer<T> {
...
requires(!trait::Pointer<T> && !trait::WeakPtr<T>) struct object_pointer<T> {
  static obj_ptr get(const T &t) {
...
public:
  ...
  template <typename U>
  requires(!std::same_as<std::decay_t<U>,
                         copy_on_write>) explicit copy_on_write(U &&x)
...
private:
  template <GroupId, typename, typename...> friend class ::sigslot::signal_base;
...
template <typename Group> class grouped_slot : public slot_state {
protected:
  explicit grouped_slot(Group const &gid) : slot_state(), m_group(gid) {}
...
template <typename Group> struct cleanable {
  virtual ~cleanable() = default;
  virtual void clean(grouped_slot<Group> *) = 0;
};

template <typename Group, typename...> class slot_base;
...
public:
  ...
  explicit slot_base(cleanable<Group> &c, group_id const &gid)
      : grouped_slot<Group>(gid), cleaner(c) {}
  ...
  template <typename C>
  requires(function_traits<C>::must_check_object) bool has_full_callable(
      const C &c) const {
...
public:
  ...
  constexpr slot(cleanable<Group> &c, F &&f, Group const &gid)
      : slot_base<Group, Args...>(c, gid), func{std::forward<F>(f)} {}
...
public:
  ...
  constexpr slot_pmf(cleanable<Group> &c, F &&f, P &&p, Group const &gid)
      : slot_base<Group, Args...>(c, gid), pmf{std::forward<F>(f)},
        ptr{std::forward<P>(p)} {}
...
public:
  template <typename F, typename P>
  constexpr slot_pmf_extended(cleanable<Group> &c, F &&f, P &&p,
                              Group const &gid)
      : slot_base<Group, Args...>(c, gid), pmf{std::forward<F>(f)},
        ptr{std::forward<P>(p)} {}
...
public:
  template <typename F, typename P>
  constexpr slot_pmf_tracked(cleanable<Group> &c, F &&f, P &&p,
                             Group const &gid)
      : slot_base<Group, Args...>(c, gid), pmf{std::forward<F>(f)},
        ptr{std::forward<P>(p)} {}
...
public:
  ...
  static constexpr bool is_thread_safe =
      !std::same_as<Lockable, detail::null_mutex>;

private:
  template <typename U>
  using cow_type =
      std::conditional_t<is_thread_safe, detail::copy_on_write<U>, U>;

  template <typename U>
  using cow_copy_type =
      std::conditional_t<is_thread_safe, detail::copy_on_write<U>, const U &>;
...
public:
  ...
  requires trait::MemberCallable<Pmf, Ptr, T...> &&
      (!trait::Observer<Ptr> && !trait::WeakPtrCompatible<Ptr>)connection
      connect(Pmf &&pmf, Ptr &&ptr, group_id gid = group_id{}) {
    using slot_t = detail::slot_pmf<group_id, Pmf, Ptr, T...>;
    auto s =
        make_slot<slot_t>(std::forward<Pmf>(pmf), std::forward<Ptr>(ptr), gid);
    connection conn(s);
  ...
  requires trait::MemberCallable<Pmf, Ptr, connection &, T...> &&
      (!trait::WeakPtrCompatible<Ptr>)connection
      connect_extended(Pmf &&pmf, Ptr &&ptr, group_id gid = group_id{}) {
  ...
  requires(!trait::Callable<Pmf, T...>) connection
      connect(Pmf &&pmf, Ptr &&ptr, group_id gid = group_id{}) {
    using trait::to_weak;
  ...
  requires trait::Callable<Callable, T...> connection
  connect(Callable &&c, Trackable &&ptr, group_id gid = group_id{}) {
    using trait::to_weak;
  ...
  requires(!trait::Callable<Obj, T...> &&
           ...
           !trait::MemFnPointer<Obj>) size_t disconnect(const Obj &obj) {
 ...
 */
template <template <typename...> typename Sig, typename Owner, GroupId Group,
          typename... Args>
class signal_interface final {
  ...
  template <typename... U> inline void operator()(U &&...args) {
    (*_sig)(std::forward<U>(args)...);
  }

  inline size_t slot_count() noexcept { return _sig->slot_count(); }

  inline void block() noexcept { _sig->block(); }

  inline void block(Group const &gid) { _sig->block(gid); }

  inline void unblock(Group const &gid) { _sig->unblock(gid); }

  inline void unblock() noexcept { _sig->unblock(); }

  inline bool blocked() const noexcept { return _sig->blocked(); }
...
public:
  using group_id = Group;
  signal_interface()
      : _sigStorage(std::in_place), _sig(std::addressof(*_sigStorage)) {}

  explicit signal_interface(signal_type *sig)
      : _sigStorage(std::nullopt), _sig(sig) {}
  ...
  inline void disconnect_all() { _sig->disconnect_all(); }
};
 ...
 */
template <typename... T> using signal = signal_base<int32_t, std::mutex, T...>;

template <GroupId Group, typename... T>
using signal_g = signal_base<Group, std::mutex, T...>;
